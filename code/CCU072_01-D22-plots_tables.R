...
df$group_item[df$group_item=="1_2_most_deprived"] <- "1-2 most_deprived"
df$group_item[df$group_item=="9_10_least_deprived"] <- "9-10 least deprived"
df$group_item[df$group_item=="5_over"] <- "5+"
df$group_item[df$group_item=="90_"] <- "90+"
...
for(analysis in sort(unique(fig3_data$analysis))){
  ...
  ggsave(paste0(outputfolder,"/ccu072_01_f3_subgroups_noethnicitynoregion_",analysis,".pdf"), width=297*2, height=180*2, limitsize=F, unit="mm", # 210
         device = cairo_pdf, onefile = TRUE,
         marrangeGrob(output.plots, 
...
##################################################
#     SUBGROUP PLOTS FOR ETHNICITY (FIG 4)       #
##################################################
