...
# MAGIC %md
# MAGIC # CCU072_01-D12-calculation_metrics_source_weight
# MAGIC
# MAGIC **Description** This notebook calculate monthly prevalence, incidence, 30-d mortality and recurrence by data sources incorporating personal weights.
# MAGIC
...
for i in period:
    ...
    tmp = spark.table(f'{dsa}.{proj}_tmp7_all_year{i}_source_recur_weightage1')
    ...
    tmp0 = (
            tmp
            .groupBy('dcode','source')                              
            .agg(
                *[f.sum(f.col(col)*f.col('weight')).alias(col) for col in list_cols ] 
                )
          )

    tmp0.write.mode('overwrite').option('overwriteSchema','true').saveAsTable(f'{dsa}.{proj}_tmp0_wide_source_all_year{i}')
    tmp0 = spark.table(f'{dsa}.{proj}_tmp0_wide_source_all_year{i}')
    print(f'tmp0 for year {i} saved')

    tmp1 = (tmp0
            .select('dcode','source',*[col for col in list_cols],*[(f.col(f'flag_prev_{monx}')/f.col(f'pop_{monx}')*100).alias(f'prev_{monx}') for monx in list_months],*[(f.col(f'flag_inc_{monx}')/f.col(f'DIFF_{monx}')*365.25*100000).alias(f'inc_{monx}') for monx in list_months],*[(f.col(f'flag_fatal_{monx}')/f.col(f'flag_inc_fatal_{monx}')*100).alias(f'fatal_{monx}') for monx in list_months],*[(f.col(f'flag_recur_{monx}')/f.col(f'DIFF_recur_{monx}')*365.25*100000).alias(f'recur_{monx}') for monx in list_months])
          )
            
        
    tmp2 = (tmp1.unpivot(['dcode','source'],[f'flag_prev_{monx}' for monx in list_months]+[f'flag_inc_{monx}' for monx in list_months]+[f'flag_fatal_{monx}' for monx in list_months]+[f'flag_recur_{monx}' for monx in list_months],'n_name','numerator')
    ...
    tmp3 = (tmp1.unpivot(['dcode','source'],[f'pop_{monx}' for monx in list_months]+[f'DIFF_{monx}' for monx in list_months]+[f'flag_inc_fatal_{monx}' for monx in list_months]+[f'DIFF_recur_{monx}' for monx in list_months],'n_name','denominator')
    ...
    tmp4 = (tmp1.unpivot(['dcode','source'],[f'prev_{monx}'for monx in list_months]+[f'inc_{monx}'for monx in list_months]+[f'fatal_{monx}' for monx in list_months]+[f'recur_{monx}' for monx in list_months],'n_name','result')
    ...
    tmpc =(tmp2.join(tmp3,['dcode','source','metric','year','month']).join(tmp4,['dcode','source','metric','year','month'])
                .withColumn('year',f.col('year').astype('int'))
                .withColumn('month',f.col('month').astype('int'))
            )
    
    print(f'All monthly weighted metrics by source for all phenos in year{i} completed') 
    
    tmpc.write.mode('overwrite').option('overwriteSchema', 'true').saveAsTable(f'{dsa}.{proj}_tmp_all_year{i}_allpos_bysource_weightage1')
...
for i in period:
  k +=1
  tmp = spark.table(f'{dsa}.{proj}_tmp_all_year{i}_allpos_bysource_weightage1')
  if k==1:tmpc = tmp
  else: tmpc = tmpc.unionByName(tmp)
tmpc.write.mode('overwrite').option('overwriteSchema', 'true').saveAsTable(f'{dsa}.{proj}_results_month_source_weightage1')
