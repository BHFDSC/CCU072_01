...
data_all_n <- data_group[data_group$metric==prevalence_value,] %>% dplyr::group_by(year, group, value) %>% dplyr::summarise(n=max(denominator)) %>% as.data.frame
print(data_all_n)
print(head(data_all_n))
data_all_n <- data_all_n %>% dplyr::group_by(year, group) %>% dplyr::summarise(n=sum(n)) %>% dplyr::group_by(year) %>% dplyr::summarise(n=max(n)) %>% as.data.frame
